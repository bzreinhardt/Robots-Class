function [pdf] = discreteLocationUpdate(pdf0,map,X,Y,measurement)
...
sigmaNS = 0.1;
sigmaEW = 0.3;
sigma = [sigmaNS; sigmaEW];
bigSigma = diag([sigmaNS,sigmaEW,sigmaNS,sigmaEW]);
angles = [pi/2 0 -pi/2 pi];
maxRange = 3;

dx = X(1,2) - X(1,1); dy = Y(end-1,1) - Y(end,1);
midX = X + dx/2; midY = Y + dy/2;

[n,m] = size(X);
%cycle through different squares
pdf = pdf0;
%go through each z and find the probability of that happening
...
for i = 1:n
    for j = 1:m
        ...
        range(isnan(range)) = 3;
        diff = measurement-range';
        p = mvnpdf(diff,0,bigSigma);
        pdf(i,j) = p*pdf(i,j);
    end
end
pdf = pdf/sum(sum(pdf));
...
%             if ~isnan(range(k)) && measurement(k) ~= maxRange
%                 p = normpdf(measurement(k),range(k),sigma(2-mod(k,2)));
